...
export default function TeacherTimetablePage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white">Teaching Schedule</h1>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Review your weekly academic course lectures, timing slots, assigned classes, and classroom locations.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
        ...
        {loading ? (
          <div className="py-12 text-center text-xs text-slate-500">Loading teaching schedule...</div>
        ) : (
          <div className="space-y-6">
            {days.map((day) => {
              ...
              return (
                <div key={day} className="border-b border-slate-100 dark:border-slate-850 pb-4 last:border-b-0 last:pb-0">
                  ...
                  {periods.length === 0 ? (
                    <p className="text-xs text-slate-400 italic">No classes assigned on this day.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                      {periods.map((p, idx) => (
                        ...
                        >
                          ...
                          <span className="block font-bold truncate">{p.subjectName}</span>
